...
# =================== CONFIG ===================
PERIOD = "120d"               # Historical period to scan
...
st.caption("Automatically loads your stock list, scans with ML model, and shows top 5 BUY picks. (Educational use only)")
...
def safe_download(sym, period=PERIOD, tries=4, sleep_base=5):
    ...
    for attempt in range(tries):
        try:
            ...
            st.info(f"⏳ Rate limit for {sym}, retrying in {wait:.1f}s...")
            time.sleep(wait)
...
def model_build():
    """Construct an ensemble ML model."""
    rf = Pipeline([
...
def evaluate_symbol(sym):
    """Train model & return BUY confidence for symbol."""
...
def batch_prefilter(universe, batch_size=BATCH_SIZE, period="30d"):
    """Quickly filter symbols with enough data before full scan."""
    valid = []
    for sym in universe:
        try:
            hist = yf.Ticker(sym).history(period=period)
            if not hist.empty and hist["Close"].std() > 0:
                valid.append(sym)
        except Exception:
            continue
        if len(valid) % batch_size == 0:
            time.sleep(0.5)
    return valid
...
# =================== RUN SCAN ===================
start_btn = st.button("🚀 Run Full Market Scan")

if start_btn:
    # Prefilter for sanity
    st.info("🔍 Prefiltering stocks with sufficient data...")
    tickers = batch_prefilter(tickers, BATCH_SIZE)
    st.success(f"✅ {len(tickers)} stocks passed basic data filter")
    ...
    st.info("👆 Click 'Run Full Market Scan' to start scanning all tickers automatically.")
